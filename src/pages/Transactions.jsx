...
export default function Transactions() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-black dark:text-white mb-1 mt-4">Search Description or Notes</label>
        <input
          type="text"
          placeholder="e.g. rent, electricity, refund"
          value={search}
          ...
          <tbody>
            {filtered.map(tx => (
              <tr key={tx.id} className="border-b border-gray-200 dark:border-gray-700">
                ...
                <td className="p-2">
                  {tx.receipt ? (
                    <img src={tx.receipt} alt="receipt" className="h-10 rounded" />
                  ) : (
                    "-"
                  )}
                </td>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan="8" className="p-2 text-center text-gray-500 dark:text-gray-400">No transactions match your filters.</td>
              </tr>
            )}
          </tbody>
        </table>
